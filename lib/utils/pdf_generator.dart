...
Future<void> generateAndSavePdf({
  ...
  try {
    print('\n=== Generating PDF and CSV with Group-Based Counts ===');
    ...
    final baseFilename = '${itemName}_${lotNumber}_$timestamp';
    
    // First, save CSV as backup
    try {
      await _generateCSV(
      ...
      // Continue with PDF generation even if CSV fails
    }

    // Then attempt PDF generation
    try {
      ...
      // If PDF fails, return CSV path as fallback
      final csvPath = join(selectedDirectory, '${baseFilename}.csv');
      if (await File(csvPath).exists()) {
        onSuccess(csvPath);
      } else {
        onError('Failed to generate both PDF and CSV');
      }
    }
  } catch (e) {
...
Future<String> _generateCSV({
  ...
  final counts = await DatabaseHelper().getGroupedScanCounts(itemName, poNo);
  ...
  csvContent.writeln('Generated by:,$userName');
  csvContent.writeln('');
  
  // Write item details
  csvContent.writeln('Item Details');
  csvContent.writeln('Item Name,$itemName');
  csvContent.writeln('Lot Number,$lotNumber');
  csvContent.writeln('Content,$content');
  csvContent.writeln('P.O Number,$poNo');
  csvContent.writeln('Total QTY,$quantity');
  csvContent.writeln('Remarks,$remarks');
  csvContent.writeln('');
  
  // Write summary counts
  csvContent.writeln('Summary Counts');
  csvContent.writeln('Total QTY,$quantity');
  csvContent.writeln('Total Inspection QTY,${counts['inspectionQty']}');
  csvContent.writeln('Total Good Count,${counts['goodCount']}');
  csvContent.writeln('Total No Good Count,${counts['noGoodCount']}');
  csvContent.writeln('');
  
  // Write box quantities
  csvContent.writeln('Box Quantities');
  for (var box in boxQuantities) {
    final DateTime createdAt = DateTime.parse(box['createdAt']);
    final String formattedDate = 
        '${createdAt.year}-${createdAt.month.toString().padLeft(2, '0')}-${createdAt.day.toString().padLeft(2, '0')} '
        '${createdAt.hour.toString().padLeft(2, '0')}:${createdAt.minute.toString().padLeft(2, '0')}';
    csvContent.writeln('Box ($formattedDate): Target QTY,${box['qtyPerBox']}');
  }
  csvContent.writeln('');
  
  // Write results table header
  csvContent.writeln('Results Table');
  csvContent.writeln('No.,Content,Result');
  
  // Process table data similar to PDF
  ...
  // Sort and process rows
  for (var row in tableData) {
    final key = '${row['sessionId']}_${row['groupNumber']}';
    final groupRows = groupedRows[key]!;
    final position = groupRows.indexWhere((r) => r['id'] == row['id']) + 1;
    final displayGroupNumber = position == 1 ? row['groupNumber'].toString() : '';
    
    // Check if any scan in group is No Good
    final hasNoGood = groupRows.any((r) => r['result'] == 'No Good');
    final result = hasNoGood ? 'No Good' : row['result'];
    
    csvContent.writeln('$displayGroupNumber,${row['content']},$result');
  }
  
  // Save CSV file
  final csvFile = File(join(selectedDirectory, '${baseFilename}.csv'));
...
Future<String> _generatePDF({
  ...
  try {
    ...
    final pdfPath = join(selectedDirectory, '${baseFilename}.pdf');
